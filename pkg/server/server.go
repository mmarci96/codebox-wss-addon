...
func Setup(cfg *config.Config) *gin.Engine {
	...
	r.GET(cfg.WsEndpoint, WsHandler(cm, config))
	r.GET("/secret", GetSecretHandler(cm, config))
...
func GetSecretHandler(cm *ClientManager, conf *config.Config) gin.HandlerFunc {
	username := "Should get it from ctx"
	return func(ctx *gin.Context) {
		...
		if root != nil {
			...
			log.Println("User: ", username, " requested secret")
			...
			ctx.JSON(http.StatusBadRequest, errJSON)
